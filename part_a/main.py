...
class MainApp(QtWidgets.QMainWindow,From_Main):
    def __init__(self):
        ...
        self.combos=[self.FT_component1,self.FT_component_2,self.FT_component_3,self.FT_component_4,self.FT_component_5,self.FT_component_6,self.FT_component_7]
         # hide
    ...
    def connect_func(self):
        ...
        logger.info("open buttons are connected to their functions successfully")

        self.FT_component1.currentTextChanged.connect(lambda: self.choose_FT(0))
        self.FT_component_2.currentTextChanged.connect(lambda: self.choose_FT(1))
        self.FT_component1.setCurrentText("FT Magnitude")
        self.FT_component_2.setCurrentText("FT Magnitude")
        
        self.FT_component_3.currentTextChanged.connect(lambda: self.mixer(False))

        # self.FT_component_6.setCurrentText("Magnitude")
        # self.FT_component_7.setCurrentText("Phase")
        self.FT_component_6.currentTextChanged.connect(lambda: self.update_mode(0, False))
        self.FT_component_7.currentTextChanged.connect(lambda: self.update_mode(1,True))
        

        self.FT_component_4.setCurrentText("Image1")
        self.FT_component_5.setCurrentText("Image2")
        self.FT_component_4.currentTextChanged.connect(lambda: self.mixer(False))
        self.FT_component_5.currentTextChanged.connect(lambda: self.mixer(False))
        logger.info("FT_componentes are connected to their functions successfully")
    ...
    def mixer (self, flag): 
        logger.info("the mode used in mixing the images is "+str(self.mode)+" for component "\
            +str(self.FT_component_6.currentText()) +" for " + str(self.FT_component_4.currentText())+\
                 " and component "+str(self.FT_component_7.currentText())+" for "+str(self.FT_component_5.currentText()) )
        if self.mode in self.modes:
            self.imag1=self.images[self.FT_component_4.currentIndex()]
            self.imag2=self.images[self.FT_component_5.currentIndex()]
            data=self.imag1.mix(self.imag2,self.gain1,self.gain2,self.mode)
            self.draw_img(self.FT_component_3.currentIndex()+4,data)
            logger.info("the mixer worked successfully")
        else: 
            logger.info("the mixer did not work please choose the components properly")
            if (flag):
                self.msg.setWindowTitle("Error ")
                self.msg.setText("please component properly")
                self.msg.setIcon(QMessageBox.Warning)
                self.msg.exec_()
